...
%             of the depth samples.
        ...
        function success = set_led_strip_uniform(obj, color, waitForConfirmation)
            arguments
                obj QLabsQCar2
...
%             Sets all 33 LEDs on the QCar to a single uniform RGB color.
%             color: [R, G, B] on a 0.0-1.0 scale. Values > 1 enhance glow.
            ...
            obj.c.classID = obj.ID_QCAR;
            obj.c.actorNumber = obj.actorNumber;
            obj.c.actorFunction = obj.FCN_QCAR_SET_LED_STRIP_UNIFORM;
            ...
            if (obj.qlabs.send_container(obj.c))
                if waitForConfirmation
                    rc = obj.qlabs.wait_for_container(obj.ID_QCAR, obj.actorNumber, obj.FCN_QCAR_SET_LED_STRIP_UNIFORM_ACK);
                    if isempty(rc)
                        return
                    else
                        success = true;
        ...
        function success = set_led_strip_individual(obj, color, waitForConfirmation)
            arguments
                obj QLabsQCar2
                color (:,3) single
                waitForConfirmation logical = true
...
%             Sets each of the 33 LEDs individually.
%             color: float array of shape [33, 3]. Each row is [R, G, B].
            ...
            if size(color, 1) ~= 33
                fprintf('set_led_strip_individual requires a 33x3 color array\n');
                return
            end

            obj.c.classID = obj.ID_QCAR;
            obj.c.actorNumber = obj.actorNumber;
            obj.c.actorFunction = obj.FCN_QCAR_SET_LED_STRIP_INDIVIDUAL;

            payload = uint8([]);
            for i = 1:33
                payload = [payload ...
                           flip(typecast(single(color(i,1)), 'uint8')) ...
                           flip(typecast(single(color(i,2)), 'uint8')) ...
                           flip(typecast(single(color(i,3)), 'uint8'))];
            end
            obj.c.payload = payload;
            obj.c.containerSize = obj.c.BASE_CONTAINER_SIZE + length(obj.c.payload);
            ...
            if (obj.qlabs.send_container(obj.c))
                if waitForConfirmation
                    rc = obj.qlabs.wait_for_container(obj.ID_QCAR, obj.actorNumber, obj.FCN_QCAR_SET_LED_STRIP_INDIVIDUAL_ACK);
                    if isempty(rc)
                        return
                    else
                        success = true;
